...
let isUpper (s:string) = String.forall (fun c -> (int c < 0x61 || int c > 0x7a)) s

(* returns true if all letters of s are lower case *)
let isLower (s:string) = String.forall (fun c -> (int c < 0x41 || int c > 0x5a)) s
...
let repeat (s:string) (num : int) = String.replicate num s

(* prepends vs.[0] to string s *)
let prepend (s:string) (prefix : string) = prefix + s

(* appends vs.[0] to string s *)
let append (s:string) (suffix : string) = s + suffix
