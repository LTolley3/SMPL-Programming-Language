...
\lhead{\YOURID}
\chead{SMPL Programming Language Specification}
\rhead{\today}
...
<number>        ::= n in N
    ...
    | NOP
	   ...
	   Two additions to the language we didn't manage to implement were the ability to read in text files as input strings and user-defined functions. These would be the next features we'd add to the language if we were to continue working on it. We'd also write more built-in functions like anagram.
